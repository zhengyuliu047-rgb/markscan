...
默认会优先展示并自动采集新同步商品中命中这些关键词的商品：`team`、`plus`、`日抛`。命中排除词（如 `非日抛`、`接码`、`充值`、`token` 等）的商品不会自动启用，并会从价格看板中过滤。
...
后台“同步商品”会扫全店目录、更新分类和商品资料，并给已启用商品写入价格/库存快照；“立即采集”只刷新已启用商品，适合日常更新价格看板。

Nuxt 后端默认会启动内置定时任务：每次只处理一个启用中的店铺任务，完成后等待 1 分钟再处理下一个。默认每个店铺 6 小时自动同步一次目录，其余轮次按最近采集尝试时间从旧到新采集价格。
